...
const product = computed(() => {
    ...
    return detail.find((item) => item.id === Number(productId.value)) || null;
});
...
const name_tw = computed(() => {
    return product.value?.name_tw || "";
});
const mainImage = computed(() => {
    return `${basePath}${product.value?.mainImage || ""}`;
});
...
useHead({
    ...
    meta: [
        ...
        { property: "og:title", content: name_tw.value },
        { property: "og:image", content: mainImage.value }
    ],
    link: [
        {
            rel: "canonical",
...
const selectedColor = computed(() => {
    if (!product.value || !product.value.colors[selectedColorIndex.value])
        return null;
...
// 切換顏色
const selectColor = (
    color: NonNullable<typeof product.value>["colors"][0],
    colorIndex: number
) => {
...
const selectThumbnail = (
    thumbnail: NonNullable<typeof product.value>["thumbnails"][0],
                            ...
                            <div class="pdt_detail-thumbnail_list">
                                <ul class="cf lis-n">
                                    <li
                                        v-for="(
                                            thumbnail, index
                                        ) in product.thumbnails"
                                        :key="thumbnail.image"
                                        class="pdt_detail-thumbnail_item"
                                        :class="{
                                            'js-active': thumbnail.isActive
                                        }"
                                        @click="
                                        ...
                                        <span class="cnt">
                                            <span
                                                ...
                                                ▪ {{ feature
                                                    ...
                                                    v-if="index as number < Number(product.features.length - 1)" />
                                            </span>
                                        </span>
                                    </div>
                                </div>
                                ...
                                <div class="feature_icon">
                                    <ul class="cf lis-n feature_icon_list">
                                        <li
                                            ...
                                            <figure
                                                class="feature_icon_item-pic">
                                                <div class="pic">
                                                    <div
                                                        class="image-loading-wrapper icon-wrapper">
                                                        <div
                                                            class="image-skeleton"></div>
                                                        <img
